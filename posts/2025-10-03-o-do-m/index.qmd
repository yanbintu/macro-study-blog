...
问题在于为何会有这样一个看起来有些奇怪的 TVC ？一个不严格但 heuristic 的想法是考虑上述问题先是有限的 T 期问题，且增加一个终点约束 $x_{T+1} \ge 0$，complementary slackness 要求在终点时有 $\lambda_T x_{T+1} = 0$，令 $T \to \infty$ 即得 [(6)](#eq-06)。
...
最后值得额外说明的是，鉴于时间的特殊地位，我们依照惯例将时间 $t$ 记作下标，但 $V_t(x_t)$ 和 $h_t(x_t)$ 实际上是 $V(t,x_t)$ 和 $h(t,x_t)$，即时间 $t$ 在这里实际是一个状态变量，因此从这个意义上讲非平稳性的问题本身并没有特殊之处。接下来我们将看到推广到无限期会将我们的问题变为平稳的，这也是为什么我们会偏爱一个看起来更难处理的无限期问题的技术原因。
...
注意到此时正是由于我们的问题是无限期的，括号内的优化问题与原始问题因此完全相同（没有下标 $t$，因此括号内的值函数 $V(x_{t+1})$ 和括号外的值函数 $V(x_t)$ 是同一个函数），唯一的区别只在于初始状态 $x$ 不同，于是我们得到了最终的 Bellman equation：
...
上式进一步说明了为什么对于非平稳问题我们需要把时间 $t$ 作为状态变量，而平稳问题则不需要。代入决策者想象我们递归到了某一个时点进行最优化，对于平稳问题来讲，我们面对的是和其他任一时刻相同的问题，唯一的区别只在于当前的初始状态 $x$ 不一样，也即 $x$ 已经完全概况了我们当前的状态，它是我们当前状态的一个 “充分统计量”。但对于一个非平稳问题来讲，只知道状态变量 $x$ 的值并不足够，例如对于上面的有限期问题而言，必须知道我们处在哪一个时点才能知道还剩多少期。总结来讲，对于平稳问题而言，时间 $t$ 的信息没有价值，因此有时我们会看到有些教材将平稳的 Bellman euqation 写成：

$$
\begin{align*}
    V(x) = &\max_{ \{x', y\} } u(x, y) + \beta V(x')
    \\[1ex]
    \text{s.t.} \quad &\ x' = g(x, y) \\
    &\ x' \in X, \enspace y \in Y \\
    &\ x \ \text{is given}
\end{align*}
$$ {#eq-15}

关键问题是我们上面不过是把同一个问题的 sequencial form 改写成了 recursive form，他是否带来了本质的提升？
